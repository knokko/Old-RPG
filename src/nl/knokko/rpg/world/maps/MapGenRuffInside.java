...
public class MapGenRuffInside {
	...
	private static class House extends MapGenBase{
		...
		private House(int houseDoor, int sx, int sy){
			...
			if(text != null)
				entities.add(new Villager.RuffVillager(new Point(60, 60), Game.game, text));
			else if(door == 23)
				entities.add(Villager.ruffShaman(new Point(90, 90), Game.game));
		}
	...
	public static class Shop extends House {
		...
		public Shop(int door){
			super(door, 12, 7);
			fill2(1, 2, 10, 2, 9);
			entities.add(new Villager(new Point(180, 30), Game.game, false));
			if(door == 1)
	...
	public static final SpecialText textForDoor(int door){
		...
		if(door == 14){
			if(Game.game.quests.hasQuest("shaman"))
