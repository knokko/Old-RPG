...
public abstract class MapGenBase {
	...
	public Point getStartPoint(MapGenBase previousMap){
		Vector2f point = GuiWorldMap.positionForMap(previousMap);
		Vector2f own = GuiWorldMap.positionForMap(this);
		float distanceX = point.x - own.x;
		float distanceY = -(point.y - own.y);
	...
	public void saveData(){
		try {
			...
			MainClass.console.println("Failed to save map data:");
	...
	public void loadData(){
		try {
			...
			MainClass.console.println("Failed to load map data:");
			ex.printStackTrace(MainClass.console);
			MainClass.console.println();
		}
		int t = 0;
