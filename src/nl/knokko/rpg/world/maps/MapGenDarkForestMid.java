...
public class MapGenDarkForestMid extends MapGenBase {
	...
	public ArrayList<Entity> getEnemies(){
		...
		Game g = Game.game;
		Point p = new Point();
		enemies.add(new DarkSoul(g, p, 25));
		enemies.add(new DarkSoul(g, p, 26));
		enemies.add(new DreamEater(g, p, 23));
		enemies.add(new DreamEater(g, p, 24));
		enemies.add(new DreamEater(g, p, 25));
		enemies.add(new Drock(g, p, 25));
		enemies.add(new Drock(g, p, 26));
		enemies.add(new Vampire(g, p, 24));
		enemies.add(new Vampire(g, p, 25));
		return enemies;
	}
	...
	public void loadData(){
		...
		try {
			...
			Game.console.println("Failed to load dark forest data:");
	...
	public void saveData(){
		...
		try {
			...
			Game.console.println("Failed to save dark forest data:");
			ex.printStackTrace(Game.console);
			Game.console.println();
		}
	}
	...
	public void update(World world){
		if(Bosses.bosses.contains("voidlings") && Game.game.specialItems.hasItem(SpecialItems.CURSED_EYE)){
			if(new Point(3000, 3000).distance(world.game.player.position) < 90 && !new Point(3000, 300).equals(world.game.player.position)){
				Game g = world.game;
				Point p = new Point(-10000, -10000);
				g.currentGUI = new GuiChat(g, false).setFinishGui(new GuiBattle(g, BackGrounds.dark_forest, g.players(), new Entity[]{new VoidLing(g, p, 30), new VoidLing(g, p, 30), new VoidLing(g, p, 30), new VoidLing(g, p, 30)}));
			}
		}
	}
