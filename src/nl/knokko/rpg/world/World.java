...
public class World {
	...
	public static final float factorX = 0.009375f * 4;
	public static final float factorY = 0.016667f * 4;
	...
	*/
	...
	public void render(){
		...
		while(y < worldSizeY){
			int x = 0;
			while(x < worldSizeX){
				Tiles.fromId(tiles[x][y]).render(new Point(x * 30, y * 30));
				++x;
			}
			++y;
		}
		MainClass.player.render();
		...
		while(y < worldSizeY){
			int x = 0;
			while(x < worldSizeX){
				Tiles.fromId(tiles2[x][y]).render(new Point(x * 30, y * 30));
				++x;
			}
			++y;
		}
		...
		while(y < worldSizeY){
			int x = 0;
			while(x < worldSizeX){
				Tiles.fromId(tiles3[x][y]).render(new Point(x * 30, y * 30));
				++x;
			}
			++y;
		}
	...
	public void startRandomFight(){
		if(map.allowRandomFights()){
			...
			MainClass.currentGUI = new GuiBattle(map.getFightBackGround(), new Entity[]{MainClass.player, MainClass.player2, MainClass.player3}, enemies);
		}
	}
	...
	public void loadData(){
		try {
			...
			MainClass.console.println("Failed to load the current map; Runia will be build:");
	...
	public void saveData(){
		try {
			...
			MainClass.console.println("World.saveData(): Failed to save the map:");
			MainClass.console.println();
