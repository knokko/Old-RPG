...
public class Player extends Entity implements IPlayer{
	...
	public void saveData(){
		try {
			...
			if(getName().matches("Bart")){
				...
				writer.println("player3: " + game.player3);
				writer.close();
			}
		} catch (FileNotFoundException e) {
			Game.console.println("Player.saveData(): Failed to save the player data of " + getName() + ":");
	...
	public void loadBattleData(BufferedReader reader){
		try {
			...
			Game.console.println("failed to load player battle data:");
			e.printStackTrace(Game.console);
			Game.console.println();
		} 
	}
			...
			if(getName().matches("Bart")){
				...
				if(line != null){
					...
					try {
						...
						game.player3 = (Player) playerC.getConstructor(Game.class, Point.class).newInstance(game, new Point());
					} catch(Exception ex){
						Game.console.println("There is no player 3: " + ex.getLocalizedMessage());
						Game.console.println();
					}
				}
			}
		} catch(Exception ex){
			Game.console.println("Failed to load the player data:");
	...
	public void update(){
		...
		if(portalCooldown > 0)
			--portalCooldown;
		if(game.world != null && game.world.map != null && getName().matches("Bart")){
			if(game.world.map.getBackGroundColor() == null){
				if(position.x >= game.world.map.minPoint().x + 810 && position.x <= game.world.map.maxPoint().x - 810)
					game.camera.x = position.x;
				if(position.y >= game.world.map.minPoint().y + 450 && position.y <= game.world.map.maxPoint().y - 450)
					game.camera.y = position.y;
			}
			else {
				game.camera = position.getLocation();
			}
		}
	}
	...
	public boolean move(int x, int y){
		ArrayList<Portal> portals = game.world.portals;
		int t = 0;
		...
		t = 0;
		while(t < game.world.conversations.size()){
			Conversation con = game.world.conversations.get(t);
			if(position.equals(con.position)){
				game.currentGUI = con.messages;
				game.world.conversations.remove(t);
				return false;
			}
			++t;
		}
	...
	public void updateMap(){
		try {
			boolean[][] b = game.world.map.exploredFields;
