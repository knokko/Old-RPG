...
import nl.knokko.rpg.items.ItemBoots;
import nl.knokko.rpg.items.ItemChestplate;
import nl.knokko.rpg.items.ItemEquipment;
import nl.knokko.rpg.items.ItemHelmet;
import nl.knokko.rpg.items.ItemLeggings;
...
public class ModelEquipment extends Model {
	...
	protected Texture equipmentTexture;
	protected Texture equipmentTextureLeft;
	protected Texture equipmentTexture2;
	protected Texture equipmentTextureLeft2;
	
	protected ArmorType type;

	public ModelEquipment(String sprite, ItemEquipment equipment, Point base, Point rotate, Entity owner, boolean isGhost, ArmorType type) {
	...
	public ModelEquipment(String sprite, Point base, Point rotate, Entity owner, ArmorType type){
		this(sprite, base, rotate, owner, false, type);
	}
	
	public ModelEquipment(String sprite, Point base, Point rotate, Entity owner, boolean isGhost, ArmorType type){
		this(sprite, null, base, rotate, owner, isGhost, type);
	}
	...
	protected void render(Vector2f screen){
		...
		if(equipmentTexture != null)
			renderTexture(screen, equipmentTexture, equipmentTextureLeft);
		if(equipmentTexture2 != null)
			renderTexture(screen, equipmentTexture2, equipmentTextureLeft2);
	}
	
	protected void findTexture(){
		if(type == ArmorType.ARM){
			ItemChestplate plate = entity.getChestPlate();
			if(plate != null){
				equipmentTexture = plate.getRotateableArmTexture(false);
				equipmentTextureLeft = plate.getRotateableArmTexture(true);
			}
			else {
				equipmentTexture = null;
				equipmentTextureLeft = null;
			}
		}
		if(type == ArmorType.LEG){
			ItemLeggings leggings = entity.getLeggings();
			ItemBoots boots = entity.getBoots();
			if(leggings != null){
				equipmentTexture = leggings.getRotateableTexture(false);
				equipmentTextureLeft = leggings.getRotateableTexture(true);
			}
			else {
				equipmentTexture = null;
				equipmentTextureLeft = null;
			...
			if(boots != null){
				equipmentTexture2 = boots.getRotateableTexture(false);
				equipmentTextureLeft2 = boots.getRotateableTexture(true);
			}
			else {
				equipmentTexture2 = null;
				equipmentTextureLeft2 = null;
			}
		}
		if(type == ArmorType.CHESTPLATE){
			ItemChestplate plate = entity.getChestPlate();
			if(plate != null){
				equipmentTexture = plate.getRotateableTexture(false);
				equipmentTextureLeft = plate.getRotateableTexture(true);
			}
			else {
				equipmentTexture = null;
				equipmentTextureLeft = null;
		...
		if(type == ArmorType.HELMET){
			ItemHelmet helmet = entity.getHelmet();
			if(helmet != null){
				equipmentTexture = helmet.getRotateableTexture(false);
				equipmentTextureLeft = helmet.getRotateableTexture(true);
			}
			else {
				equipmentTexture = null;
				equipmentTextureLeft = null;
	...
	public static enum ArmorType {
		...
		HELMET(new Point(4, 6)),
		CHESTPLATE(new Point(4, 6)),
		ARM(new Point(1, 11)),
		LEG(new Point(1, 1));
		
		public final Point rotationPoint;
		
		ArmorType(Point rotationPoint){
