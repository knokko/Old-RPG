...
public abstract class Entity {
	...
	public Entity(Point spawn) {
		random = new Random();
		position = spawn;
		elementStats = new ElementStats();
		addBody();
		addAI();
		texture = Resources.getTexture("sprites/tiles/" + "white rock" + ".png");
		//TODO just as test
	}
	
	public Entity(Point position, int level){
		this(position);
		this.level = level;
	}
	...
	public Entity clone() {
		try {
			...
			if(this instanceof Player)
				entity = getClass().getConstructor(Point.class).newInstance(position.getLocation());
			else
				entity = getClass().getConstructor(Point.class, int.class).newInstance(position.getLocation(), level);
			entity.effects = cloneEffects();
			entity.armor = armor;
			return entity;
		} catch(Exception ex){
			MainClass.console.println("failed to clone Entity:");
	...
	public void render(){
		...
		if(rotation != 3 && weapon != null){
			//weapon.item.renderOnEntity(this, screen, screen2);
		}
		for(ItemArmor peace : armor){
			if(peace != null){
				//peace.renderOnEntity(this, screen, screen2);
			}
		}
		if(rotation != 3 && weapon != null){
			//weapon.item.renderOnEntity(this, screen, screen2);
	...
	public boolean move(int x, int y){
		if(moveX == 0 && moveY == 0){
			Tile goal = MainClass.world.getTile(position.x + x, position.y + y + moveY, true);
			Tile goal2 = MainClass.world.getTile(position.x + x, position.y + y, false);
			if(x > 0){
				rotation = 0;
			}
			else if(x < 0){
			...
			if(goal.walkOver(target, this) && goal2.collide(target, this)){
				...
				if(MainClass.player.position.equals(target))
					return false;
				moveX = (byte) x;
				moveY = (byte) y;
				return true;
	...
	public void update(){
		...
		if(moveX == 0 && moveY == 0)
			moveSpeed = 100 / DisplayManager.FPS_CAP;
	}
	...
	public void updateTexture(){
		if(rotation == 3)
			texture = Resources.getTexture(getTexture() + "_front.png");
		if(rotation == 1)
			texture = Resources.getTexture(getTexture() + "_behind.png");
	}
	
	public ItemHelmet getHelmet(){
		return (ItemHelmet) armor[0];
	}
	...
	public ItemLeggings getLeggings(){
		return (ItemLeggings) armor[2];
	}
	
	public ItemBoots getBoots(){
		return (ItemBoots) armor[3];
	}
	...
	public void applyDifficulty(){
		maxHealth *= MainClass.difficulty;
		currentHealth *= MainClass.difficulty;
		maxMana *= MainClass.difficulty;
		currentMana *= MainClass.difficulty;
		spirit *= MainClass.difficulty;
		strength *= MainClass.difficulty;
	}
	...
	protected void addHumanBody(){
		String n = getTexture().toLowerCase();
		models.add(new ModelEquipmentLeg(n + "/leg.png", new Point(0, -5), new Point(1, 1), this, ArmorType.LEG));
		models.add(new ModelEquipment(n + "/arm.png", new Point(0, 4), new Point(1, 11), this, ArmorType.ARM));
		models.add(new ModelEquipment(n + "/body.png", new Point(0, 1), new Point(4, 6), this, ArmorType.CHESTPLATE));
		models.add(new ModelEquipment(n + "/head.png", new Point(0, 8), new Point(4, 6), this, ArmorType.HELMET));
		models.add(new ModelEquipmentArm(n + "/arm.png", new Point(0, 4), new Point(1, 11), this, ArmorType.ARM));
		models.add(new ModelEquipmentLeg(n + "/leg.png", new Point(0, -5), new Point(1, 1), this, ArmorType.LEG));
		isHuman = true;
	}
	...
	protected void addGhostBody(){
		String n = getTexture().toLowerCase();
		models.add(new ModelEquipmentLeg(n + "/leg.png", new Point(15, 18), new Point(1, 1), this, true, ArmorType.LEG));
		models.add(new ModelEquipment(n + "/arm.png", new Point(17, -2), new Point(2, 12), this, true, ArmorType.ARM));
		models.add(new ModelEquipment(n + "/body.png", new Point(13, 6), new Point(4, 6), this, true, ArmorType.CHESTPLATE));
		models.add(new ModelEquipment(n + "/head.png", new Point(13, 0), new Point(4, 5), this, true, ArmorType.HELMET));
		models.add(new ModelEquipmentArm(n + "/arm.png", new Point(17, -2), new Point(2, 12), this, true, ArmorType.ARM));
		models.add(new ModelEquipmentLeg(n + "/leg.png", new Point(15, 18), new Point(1, 1), this, true, ArmorType.LEG));
		isHuman = true;
	}
	...
	protected Model getModel(String model){
		try {
			...
			MainClass.console.println("Entity " + this + " doesn't have model " + model + "!");
			return null;
		} 
	}
