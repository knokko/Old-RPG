...
public final class SpecialItems {
	...
	public SpecialItems(Game theGame){
		game = theGame;
	}
	...
	public void addItem(String item){
		...
		if(!items.contains(item)){
			ArrayList<Quest> quests = Game.game.quests.quests;
			int t = 0;
			while(t < quests.size()){
	...
	public void saveData(){
		try {
			...
			Game.console.println("SpecialItems.saveData(): Failed to save special items:");
	...
	public void loadData(){
		try {
			...
			Game.console.println("Failed to load special items:");
			ex.printStackTrace(Game.console);
			Game.console.println();
		}
	}
