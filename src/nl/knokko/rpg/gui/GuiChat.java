...
public class GuiChat extends Gui {
	...
	public static void loadData(){
		try {
			...
			MainClass.console.println("Failed to load the conversations; all conversations will be added:");
	...
	public static void saveStaticData(){
		try {
			...
			MainClass.console.println("GuiChat.saveStaticData(): Failed to save the conversations:");
			MainClass.console.println();
			ex.printStackTrace(MainClass.console);
			MainClass.console.println();
		}
	...
	public static GuiChat fromString(String chat){
		try {
			...
			MainClass.console.println("Failed to get conversation " + chat);
			MainClass.console.println();
			ex.printStackTrace(MainClass.console);
			return null;
		}
	...
	public GuiChat(boolean canCancel, boolean paintWorld, Point basePoint, SpecialText... messages) {
		super(null);
		cancel = canCancel;
		texts = messages;
	...
	public GuiChat(boolean cancel, boolean paintWorld, SpecialText...texts){
		this(cancel, paintWorld, new Point(100, 500), texts);
	}
	
	public GuiChat(boolean cancel, SpecialText... specialTexts){
		this(cancel, false, specialTexts);
	}
	...
	public void paint(Graphics gr){
		if(paintTheWorld){
			gr.setColor(MainClass.world.backGround);
			gr.fillRect(0, 0, MainClass.getWidth(), MainClass.getHeight());
			MainClass.world.paint(gr);
			paintTheWorld = false;
		}
		super.paint(gr);
	...
	*/
	...
	public void finish(){
		MainClass.currentGUI = finishGui;
		if(name != null)
			conversations.remove(this);
		MainController.timeOut = 10;
	}
	...
	public GuiChat clone(){
		return new GuiChat(paintTheWorld, cancel, texts);
	}
