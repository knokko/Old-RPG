...
public class GuiChest extends Gui {
	...
	public GuiChest(String texture, Chest theChest, Player thePlayer) {
		super(texture);
		chest = theChest;
		player = thePlayer;
		...
		while(t < chest.specialItems.size()){
			MainClass.specialItems.addItem(chest.specialItems.get(t));
			++t;
		}
	}
	...
	public void click(Vector2f mouse){
		...
		ItemSlot slot = player.inventory.getSlot(new Vector2f(0.03125f, 0.16666f), mouse);
		if(slot == null)
			slot = chest.inventory.getSlot(new Vector2f(0.03125f, 0.03333f), mouse);
		if(slot != null)
	...
	public void paint(Graphics gr){
		...
		gr.setFont(font());
		gr.drawString("money = " + MainClass.player.inventory.money, factorX(700), factorY(750));
	...
	*/
	...
	public void buttonClicked(Button button){
		...
		if(button.name.matches("take money")){
			MainClass.player.inventory.money += chest.inventory.money;
	...
	public void rightClick(Vector2f mouse){
		ItemSlot slot = player.inventory.getSlot(new Vector2f(0.03125f, 0.16667f), mouse);
		if(slot == null){
			slot = chest.inventory.getSlot(new Vector2f(0.03125f, 0.03333f), mouse);
		}
		if(slot != null){
