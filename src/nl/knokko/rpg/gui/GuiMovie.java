...
import static nl.knokko.rpg.utils.Utils.*;
...
public abstract class GuiMovie extends Gui {
	...
	public void paint(Graphics gr){
		...
		++tick;
		roulator += 2.5 / MainClass.fpsFactor;
		if(roulator >= 50){
			roulateText();
			roulator = 0;
		}
	...
	*/
	...
	public static class GuiIntroMovie extends GuiMovie {
		...
		public MovieObject chomper = new MovieObject(MainClass.player2, 1600, 730);
		public MovieObject chompo = new MovieObject(new Chompo(new Point(1620, 725)), 1620, 725);
		public MovieObject hurey = new MovieObject(new Hurey(new Point(200, 715), 15), 200, 715);
		public MovieObject zombie = new MovieObject(new Zombie(new Point(340, 720), 15), 340, 720);
		public MovieObject furyCaptain = new MovieObject(new FuryCaptain(new Point(1630, 730), 10), 1630, 730);
		
		public MovieObject[] furySlicers = new MovieObject[]{new MovieObject(new FurySlicer(new Point(1620, 700), 10), 1620, 700), new MovieObject(new FurySlicer(new Point(1600, 720), 10), 1600, 720), new MovieObject(new FurySlicer(new Point(1590, 740), 10), 1590, 740), new MovieObject(new FurySlicer(new Point(1620, 760), 10), 1620, 760)};
		public MovieObject[] furyDogs = new MovieObject[]{new MovieObject(new FuryDog(new Point(1650, 730), 30), 1650, 730), new MovieObject(new FuryDog(new Point(1630, 750), 30), 1630, 750), new MovieObject(new FuryDog(new Point(1620, 770), 30), 1620, 770), new MovieObject(new FuryDog(new Point(1650, 790), 30), 1650, 790)};
		
		public boolean status;
		
		public GuiBattle battle1 = new GuiBattle(BackGrounds.red_cave, new Entity[]{new Zombie(new Point(), 15), new Chompo(new Point()), MainClass.player2, new Hurey(new Point(), 15)}, new Entity[]{new FurySlicer(new Point(), 10), new FurySlicer(new Point(), 10), new FurySlicer(new Point(), 10), new FurySlicer(new Point(), 10)}, false, "intro battle"){
			...
			*/
			...
			public void click(Vector2f point){
				...
				while(t < players.length){
					Entity e = players[t];
					if(e == mark && e != null){
						if(point != null && point.x >= oldXToRelativeX(e.battlePoint.x) && point.x <= oldXToRelativeX(e.battlePoint.x + 120) && point.y >= oldYToRelativeY(e.battlePoint.y) && point.y <= oldYToRelativeY(e.battlePoint.y + 120)){
							MainClass.currentGUI = new GuiBattle1Info(e);
							e.refreshModels();
						}
					}
					++t;
			...
			@Override
			public void win(){
				MainClass.currentGUI = GuiIntroMovie.this;
			...
			class GuiBattle1Info extends GuiBattleInfo {
				...
				public void click(Vector2f mouse){
					if(button != null && button.isHit(mouse))
						buttonClicked(button);
				}
				...
				public void buttonClicked(Button button){
					if(turn == 5){
						if(button.name.matches("element stats")){
							MainClass.currentGUI = new GuiBattle1ElementStats();
							++timer;
							return;
						}
					}
		...
		public GuiBattle battle2 = new GuiBattle(BackGrounds.red_cave, new Entity[]{new Zombie(new Point(), 15), new Chompo(new Point()), MainClass.player2, new Hurey(new Point(), 15)}, new Entity[]{new FuryDog(new Point(), 30), new FuryDog(new Point(), 30), new FuryDog(new Point(), 30), new FuryDog(new Point(), 30)}, false, "intro battle"){
				...
				while(t < players.length){
					...
					if(e == mark && e != null){
						if(point != null && point.x >= factorX(e.battlePoint.x) && point.x <= factorX(e.battlePoint.x + 120) && point.y >= factorY(e.battlePoint.y) && point.y <= factorY(e.battlePoint.y + 120)){
							MainClass.currentGUI = new GuiBattleInfo(e);
						}
					}
					++t;
				}
		...
		public GuiChat conversation1 = new GuiChat(false, false, new Point(200, 300), new SpecialText("Well Chomper, this is what I wanted to show you.", "You haven't come to the Fury Mountains for nothing."), new SpecialText("That portal looks quite awesome.", "What would be behind the portal?"), new SpecialText("There will be another dimension behind it.", "But nobody has been there because the portal is unstable.", "It may close after you walk through it.", "So nobody takes the risk to enter it."), new SpecialText("Well...", "I understand nobody enters the portal.", "Shall we just enjoy this moment?"), new SpecialText("Yes, that's a good idea.")).setSpeakers(new String[]{"Chompo", "Chomper", "Chompo", "Chomper", "Chompo"}).setFinishGui(this);
		public GuiChat conversation2 = new GuiChat(false, false, new Point(200, 300), new SpecialText("Attention please.", "Our captain is going to speak."), new SpecialText("We are going to conquer the new dimension.", "But we need someone to test the portal, it can be unstable.", "Does anybody want to do that for us?")).setSpeakers(new String[]{"Fury Slicer", "Fury Captain"}).setFinishGui(this);
		public GuiChat conversation3 = new GuiChat(false, false, new Point(200, 300), new SpecialText("If nobody wants to help us, we will pick one."), new SpecialText("Hey, you can't do that!", "If nobody wants to do it, nobody goes."), new SpecialText("I am the captain, I CAN do that.", "If nobody wants to enter the portal, I will pick you."), new SpecialText("That is ridiculous!", "Nobody can steal my son from me!"), new SpecialText("If nobody comes within 5 seconds,", "I will pick that sheeky black guy.")).setSpeakers(new String[]{"Fury Captain", "Chomper", "Fury Captain", "Chompo", "Fury Captain"}).setFinishGui(this);
		public GuiChat conversation4 = new GuiChat(false, false, new Point(200, 300), new SpecialText("The 5 seconds are over.", "Go through the portal, black guy."), new SpecialText("No, I won't."), new SpecialText("That was no question, enter the portal."), new SpecialText("He said no."), new SpecialText("I don't care, he goes through the portal.", "Whether he wants it, or not.", "However, you have a choice.", "You can enter it yourself or my men ", "will throw you through the portal."), new SpecialText("I will fight if I have to.", "Don't force me to do that."), new SpecialText("If you want a fight, you can get it."), new SpecialText("I don't want to fight, only if I have to."), new SpecialText("If you don't want a fight, go through the portal."), new SpecialText("No, do it yourself."), new SpecialText("Guys, attack him."), new SpecialText("I will defend my son until the end."), new SpecialText("You will end up in prison."), new SpecialText("I will also defend that guy, this is ridiculous!")).setSpeakers(new String[]{"Fury Captain", "Chomper", "Fury Captain", "Chompo", "Fury Captain", "Chomper", "Fury Captain", "Chomper", "Fury Captain", "Chomper", "Fury Captain", "Chompo", "Fury Captain", "Hurey"}).setFinishGui(this);
		public GuiChat conversation5 = new GuiChat(false, false, new Point(200, 300), new SpecialText("Is there anyone else who likes prisons?", "You are welcome."), new SpecialText("Yes, they are right.", "You can't do this!"), new SpecialText("Believe me, I can, we have enough prisons here.")).setSpeakers(new String[]{"Fury Captain", "Zombie", "Fury Captain"}).setFinishGui(this);
		public GuiChat conversation6 = new GuiChat(false, false, new Point(200, 300), new SpecialText("Enough, show those fools some respect."), new SpecialText("Yes, captain!")).setSpeakers(new String[]{"Fury Captain", "Fury Slicers"}).setFinishGui(this);
		public GuiChat conversation7 = new GuiChat(false, false, new Point(200, 300), new SpecialText("You just killed 4 of my soldiers.", "That will be a long prison sentence.", "And that black guy still has to enter the portal."), new SpecialText("We are not going anywhere,", "you have used your power for bad things.", "Run for your life, or we will kill you too."), new SpecialText("I don't think so, I have asked for assistance.", "They can arrive any moment, you had better stop", " talking like that.")).setSpeakers(new String[]{"Fury Captain", "Hurey", "Fury Captain"}).setFinishGui(this);
		public GuiChat conversation8 = new GuiChat(false, false, new Point(200, 300), new SpecialText("If you surrender now, I will decrease the duration", "of your prison sentence.", "So, what do you say?"), new SpecialText("I had rather die!"), new SpecialText("These dogs are trained to fight until you are knock out.", "They are not trained to kill you.", "You have to pay for your behaviour, you can't just die."), new SpecialText("If that's the case, we won't kill the dogs if we defeat them."), new SpecialText("Dogs, they haven't changed their mind.", "So take them down!")).setSpeakers(new String[]{"Fury Captain", "Chompo", "Fury Captain", "Zombie", "Fury Captain"}).setFinishGui(this);

		public GuiIntroMovie() {
			super();
		}

		@Override
		public void play(Graphics gr){
			//gr.drawImage(backGround, 0, 0, MainClass.getWidth(), MainClass.getHeight(), null);
			if(status){
				play2(gr);
				return;
			}
			...
			if(tick >= 290 && tick <= 300)
				hurey.x += 5;
			if(tick == 300)
				addText("This will change soon...");
			if(tick > 300 && tick < 450){
				chomper.x -= 5.0;
				chompo.x -= 5;
			...
			if(tick == 455)
				MainClass.currentGUI = conversation1;
			if(tick >= 470 && tick <= 590){
				int t = 0;
			...
			if(tick == 600){
				MainClass.currentGUI = conversation2;
				chomper.entity.rotation = 0;
				chompo.entity.rotation = 0;
			}
			if(tick == 700)
				MainClass.currentGUI = conversation3;
			if(tick == 800)
				MainClass.currentGUI = conversation4;
			if(tick > 800 && tick < 900){
				hurey.x += 6.0;
			}
			if(tick == 900)
				MainClass.currentGUI = conversation5;
			if(tick > 900 && tick < 1000){
				zombie.x += 5.0;
			}
			if(tick == 1000)
				MainClass.currentGUI = conversation6;
			if(tick == 1001){
				MainClass.currentGUI = battle1.activate();
				furySlicers = new MovieObject[0];
			}
			if(tick == 1020)
				MainClass.currentGUI = conversation7;
			if(tick >= 1050 && tick <= 1120){
				int t = 0;
			...
			if(tick == 1121)
				MainClass.currentGUI = conversation8;
			if(tick == 1122)
				MainClass.currentGUI = battle2.activate();
			if(tick == 1123){
				status = true;
		...
		public void finish(){
			MainClass.currentGUI = null;
			MainClass.updateSpeed = 1;
			MainClass.player2.currentHealth = MainClass.player2.maxHealth;
		}
	...
	public static class MovieObject {
		...
		//Uses the old coordinate space (0 to 1600 and 0 to 900)
		public int x;
		public int y;
