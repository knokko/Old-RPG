...
public class GuiBattle extends Gui {
	...
	private GuiBattle(String texture){
		super(texture);
		this.texture = texture;
		players = new Entity[]{MainClass.player, MainClass.player2, MainClass.player3};
	...
	public GuiBattle(String texture, Entity[] players, Entity[] enemies){
		this(texture, players, enemies, true);
	}
	
	public GuiBattle(String texture, Entity[] players, Entity[] enemies, boolean startDirect){
		this(texture, players, enemies, startDirect, "battle");
	}
	
	public GuiBattle(String texture, Entity[] players, Entity[] enemies, boolean startDirect, String music) {
		super(texture);
	...
	public void paint(Graphics gr){
		gr.drawImage(image, 0, 0, MainClass.getWidth(), MainClass.getHeight(), null);
		int t = 0;
		...
		while(t < buttons.size()){
			...
			b.paint(gr);
			Point m = MainClass.getMousePosition();
			if(b.isHit(m)){
	...
	*/
	...
	public ToolBar toolbarForButton(Button button) {
		...
		toolbar.min = new Vector2f(button.minX(), button.minY() >= 0 ? button.minY() - 0.03125f : button.maxY() + 0.03125f);
	...
	private void addTextForButton(ToolBar toolbar, Button button){
		if(button.name.matches("attack")){
			...
			toolbar.text.add("Save all your progress and stop the MainClass.");
			...
			toolbar.increaseToSize(320, 0);
			toolbar.min.x += 150;
			toolbar.setColor(spell.getElement().color);
			if(toolbar.min.y + toolbar.height > 0.95f)
				toolbar.min.y = 0.95f;
		}
	...
	public void escapePressed(){
		if(MainClass.difficulty < 3){
			if(onTurn >= 0){
				super.escapePressed();
				MainClass.player.onBattleEnd();
				MainClass.player2.onBattleEnd();
				if(MainClass.player3 != null)
					MainClass.player3.onBattleEnd();
				stopMusic();
			}
		}
	}
		...
		if(Keyboard.isKeyDown(Keyboard.KEY_LEFT)){
			if(status == 0){
				if(selectedButton > 2 && selectedButton <= 5)
					--selectedButton;
				else if(selectedButton == 2 && MainClass.difficulty < 3)
					selectedButton = 6;
	...
	protected void moveMouse(){
		...
		try {
			...
			MainClass.console.println("This game is not allowed to move the mouse: " + e.getLocalizedMessage());
			bot = null;
		}
		if(selectedButton >= 0 && selectedButton < buttons.size() && bot != null){
			Button but = buttons.get(selectedButton);
			bot.mouseMove((int) ((but.midX() + 1) / 2 * Display.getWidth()), (int) ((but.midY() + 1) / 2 * Display.getHeight()));
		}
	}
	...
	public Entity randomPlayer(){
		if(MainClass.currentGUI == this){
			checkWin();
			if(MainClass.currentGUI != this){
				MainClass.console.println("GuiBattle#randomPlayer(): The battle is over; returning null...");
				return null;
			}
		}
	...
	public void lose(){
		JukeBox.stop();
		MainClass.currentGUI = new GuiGameOver();
	}
	...
	public void win(){
		...
		stopMusic();
		MainClass.currentGUI = new GuiChest("", chest, MainClass.player);
	}
	
	protected void stopMusic(){
		JukeBox.stop();
		String music = MainClass.world.map.getMusic();
		if(music != null)
			JukeBox.playMusic(music, true);
	}
	...
	public void saveData(){
		try {
			...
			MainClass.console.println("GuiBattle.saveData(): Failed to save this battle:");
	...
	public static GuiBattle loadData(){
		try {
			BufferedReader reader = new BufferedReader(new FileReader(Resources.getLoadFile() + "/battle.txt"));
			GuiBattle battle = new GuiBattle(reader.readLine().substring(9));
			...
			while(ent.contains("enemy") && t < 4){
				try {
					if(!ent.contains("null")){
						int ind = ent.indexOf("{");
						battle.enemies[t] = (Entity) Class.forName(ent.substring(9, ind)).getConstructor(Point.class, int.class).newInstance(new Point(-10000, -10000), 0);
						if(battle.enemies[t] != null)
							battle.enemies[t].loadBattleData(reader);
					}
				} catch(Exception ex){
					MainClass.console.println("failed to load battle enemy " + t + ":");
					ex.printStackTrace(MainClass.console);
					MainClass.console.println();
				}
				ent = reader.readLine();
			...
			while(ent.contains("player") && t < 4){
				try {
					if(!ent.contains("null")){
						battle.players[t] = (Entity) MainClass.class.getField("player" + (t == 0 ? "" : t + 1)).get(null);
						if(battle.players[t] != null)
							battle.players[t].loadBattleData(reader);
					}
				} catch(Exception ex){
					MainClass.console.println("failed to load battle player " + t + ":");
					ex.printStackTrace(MainClass.console);
					MainClass.console.println();
				}
				ent = reader.readLine();
			...
			while(line != null && line.contains("spell")){
				...
				try {
					...
					MainClass.console.println("failed to load battle spells:");
					ex.printStackTrace(MainClass.console);
					MainClass.console.println();
				}
				line = reader.readLine();
			...
			MainClass.console.println("failed to load battle: (this is ok if you were not in a battle while you saved)" + ex.getLocalizedMessage());
			MainClass.console.println();
			return null;
		}
	}
	...
	public void buttonClicked(Button button){
		...
		if(button.name.matches("options"))
			MainClass.currentGUI = new GuiBattleOptions();
	...
	public class GuiBattleInfo extends Gui {
		...
		*/
		...
		@Override
		public void escapePressed(){
			MainClass.currentGUI = GuiBattle.this;
		...
		public void buttonClicked(Button button){
			...
			if(button.name.matches("element stats"))
				MainClass.currentGUI = new GuiBattleElementStats();
			if(button.name.matches("equipment"))
				MainClass.currentGUI = new GuiBattleEquipment();
			if(button.name.matches("options"))
				MainClass.currentGUI = new GuiBattleOptions();
		}
			...
			*/
			...
			public void buttonClicked(Button button){
				...
				if(button.name.matches("basic stats")){
					MainClass.currentGUI = GuiBattleInfo.this;
				}
				if(button.name.matches("equipment")){
					MainClass.currentGUI = new GuiBattleEquipment();
				}
